...
// Fixed onClick function to prevent the bug
const onClick = (event) => {
    // Fresh raycasting on each click to avoid stale data
    ...
    if (intersects.length > 0) {
        ...
        if (!overlay.classList.contains('opacity-0')) {
            return; // Modal is already open, ignore this click
        }
        ...
        //     camera.position.lerp(targetPosition, 0.02); // Slower lerp for smoother animation
        //     camera.lookAt(intersectedObject.position);
        //     const distance = camera.position.distanceTo(targetPosition);
        //     if (distance < 0.5) { // Increased threshold to prevent getting stuck
        //         isAnimatingCamera = false;
        //         // Ensure camera is exactly at target position
